...
::: warning 龙虎榜是盘后数据，当日数据要等傍晚
龙虎榜由交易所**收盘后统计、傍晚才发布**。因此盘中（15:00 收盘前）查询当天日期**必然查不到当天数据**，这是数据源的正常时效，不是调用出错。

例如周一盘中查 `{ startDate: '20260730', endDate: '20260803' }`，结果只会有 07-30、07-31 两天；当晚发布后再查，08-03 的数据才会出现。

写定时任务时请把拉取时间放在**收盘当晚之后**（保险起见次日更稳），或用 [`sdk.calendar.marketStatus()`](./calendar.md) 先判断市场状态。
:::
