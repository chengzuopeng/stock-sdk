...
::: warning 北向（沪股通 / 深股通）净额已不再实时披露
上游自 2024 年起停止实时披露北向资金的净买入额，因此 `summary` 中**沪股通 / 深股通**方向的 `netInflow` / `netBuyAmount` 等净额字段**可能恒为 `0`**——这是上游不再提供，不是本 SDK 取数失败。

同一接口里**南向（港股通）**方向的净额仍有值；北向的涨跌家数、指数涨跌幅等非净额字段也照常返回，盘中即可取到当天记录。
:::
