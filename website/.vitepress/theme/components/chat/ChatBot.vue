 ...
 * 每个页面都有。后端对接 api-worker 的 EdgeOne Makers agent(POST /chat,SSE)。
 *
...
import { useChatStream, type ChatTurn } from './useChatStream'
...
const t = computed(() =>
  isEn.value
    ? {
        ...
        suggestions: [
          'How to use sdk.kline.cn? What is the default adjust?',
          'How does sdk.chips compute chip distribution?',
          'Migrating from v1: what replaces getHistoryKline?',
        ],
        ...
        searched: 'Docs searched',
        running: 'Running code in sandbox…',
        ran: 'Code executed',
      }
    : {
        title: 'stock-sdk 助手',
        ...
        suggestions: [
          'sdk.kline.cn 怎么用?复权默认值是什么?',
          '筹码分布 sdk.chips 怎么算?',
          'v1 的 getHistoryKline 在 v2 里对应什么?',
        ],
...
function clampRect() {
  ...
  // 挂载瞬间窗口可能尚未完成布局,innerWidth/innerHeight 为 0。此时按 0 去 clamp 会把
  // 窗口压成最小尺寸并吸到视口角落(maxW 变负 → 取 MIN_W,right/bottom 归 0),
  // 且之后不会自动恢复。拿不到有效尺寸就直接跳过,交给后续的 resize 事件。
...
/**
 * 工具调用状态文案。agent 检索知识库 / 跑沙箱时会推 tool_called 事件,
 * 在气泡顶部显示一行,让用户知道这几秒它在干什么(首字延迟可能有好几秒)。
 */
function toolLabel(m: ChatTurn): string {
  const tools = m.tools ?? []
  if (!tools.length) return ''
  const searched = tools.filter((x) => x === 'search_docs').length
  const ran = tools.filter((x) => x === 'run_sdk_example').length
  const parts: string[] = []
  if (searched) {
    parts.push(
      m.streaming ? t.value.searching : `${t.value.searched}${searched > 1 ? ` ×${searched}` : ''}`,
    )
  }
  if (ran) parts.push(m.streaming ? t.value.running : t.value.ran)
  return parts.join(' · ')
}
        ...
        <div ref="listEl" class="csb-list" @click="onListClick">
          ...
          >
            ...
            <div v-else class="csb-bubble csb-bubble-bot" :class="{ 'csb-err': m.error }">
              <div v-if="toolLabel(m)" class="csb-tool" :class="{ 'csb-tool-live': m.streaming }">
                <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="7"/><path d="m21 21-4.3-4.3"/></svg>
                <span>{{ toolLabel(m) }}</span>
              </div>
...
/* 截断提示 */
/* 工具调用状态条:agent 检索文档 / 跑沙箱时给用户一个交代(首字可能要等几秒) */
.csb-tool {
  display: inline-flex;
  align-items: center;
  gap: 5px;
...
.csb-tool-live svg {
  animation: csb-tool-spin 1.4s linear infinite;
}
@keyframes csb-tool-spin {
  to {
